...
namespace NHST.Controllers
{
    public class ExportRequestTurnController
    {
        ...
        {
            var list = new List<ExportRequest>();
            var sql = @"SELECT ShippingTypeInVNID,st.ShippingTypeVNName,ert.FeeInsurrane,ert.* FROM dbo.tbl_ExportRequestTurn as ert LEFT OUTER JOIN dbo.tbl_ShippingTypeVN AS st ON ert.ShippingTypeInVNID = st.ID where MainOrderID = 0 ";
            if (Status > 0)
                sql += " AND ert.Status =" + Status;
            if (!string.IsNullOrEmpty(fd))
            {
